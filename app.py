...
def _build_everything():
    """Runs once, only if the shipped model can't be loaded (e.g. a
    library-version mismatch between this machine and the one that saved it).
    Regenerating locally guarantees the pickle always matches the versions
    actually installed here."""
    import generate_data, train
    generate_data.run_generation()
    train.run_training()
...
def load():
    try:
        ...
        with st.spinner("First run: training the model for this environment (~30s)..."):
            _build_everything()
        return joblib.load("model.joblib")
...
def load_csv(path):
    if not os.path.exists(path):
        load()  # triggers _build_everything(), which creates every CSV too
